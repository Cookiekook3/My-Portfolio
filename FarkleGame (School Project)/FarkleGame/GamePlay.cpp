...
int* GamePlay::rollAgain() // Reroll function, uses rerollPosition bool array to only reroll the positions that don't score the first time
{
    for (int i = 0; i < 6; i++)
    {
...
int GamePlay::calculateScore(int* playerScore) //Scoring function
{
    ...
    if (rerollDice == 6)
    {
        cout << "Farkle! You didn't roll any scoring dice, any score gained during this turn is lost" << endl;
        score = 0;
    }
    else
    {
...
int GamePlay::eachPlayerTurn(int numPlayers, int* playerScore) //Function that handles individual player turns
{
    ...
    for (int i = 0; i < numPlayers; i++) // Repeats the turn based on how many players are playing
    {
        ...
        if (rerollDice == 0)
        {
            std::cout << "All dice are scoring dice, you get to roll all 6 dice again for more score";
            playerScore[i] += score;
            rollDice();
            displayRoll();
